...
### 做了什么
用户反馈解读区两个问题:①**粗体等 markdown 直接显示成星号**(排版丑);②输出里频繁**引"倪海夏老师说/倪师原话"**,要隐去真实人名。
- **前端 markdown 渲染**(public/index.html):新增 `mdRender(src)` —— 先 esc 防注入,再渲染子集(标题 #/##/###、粗体 `**`、斜体 `*`、无序列表 -/•/①、分隔线 ---、段落)。三处 `.reading-body` 由 `esc()` 改 `mdRender()`。配套墨韵 CSS:h2 朱砂左边线、h3 ❖ 前缀、h4 墨绿、strong 浓墨、em 朱砂、ul 朱砂点、hr 发丝线;`.reading-body` 去掉 pre-wrap。
- **隐去人名 → 易理口径**(src/ziwei.js ZIWEI_SYSTEM):把 42 处 `倪海夏/倪师/倪原话/赛博倪海夏` 全替换 —— `（倪原话）`→`（古诀云）`、`倪海夏三合派`→`三合派（南派）`、名言"天纪"句去人物化;人设首句重写为"精通正宗紫微斗数的命理大师",并加 **# 身份口径** 硬约束(不得出现"倪海夏/倪师/赛博倪海夏",引权威一律用"古诀云/经书有言/三合派主张"等)。chat.js 的 tianming 模式复用同一 ZIWEI_SYSTEM,自动生效。

### 验证
- 真解读(配 Key):人名残留 ✓ 无、易理口吻("古诀云:机月同梁格作吏人")✓ 有、断法深度保留。
- Playwright:mdRender 出 strong/h3/h4/ul/hr,无残留 `**`;截图确认墨韵排版(粗体浓墨/标题墨绿/朱砂列表点/发丝分隔)✓。
- npm test 43/43 ✓;ziwei.js 语法 OK。
- 改动未提交,等用户拍板。

### 注意
- 采"提示词改口径"(根治),未加前端正则兵底(用户选的);模型极小概率仍可能自造人名,若发现可再加一道前端过滤。
- chat.html 的 markdown 渲染未处理(用户只指天命页);如需再补。

---

## 2026-07-03(五·下午)— 命例库改 MetaSight 弹窗式(卡片列表 + 新建八字 modal)

### 做了什么
用户改用 MetaSight 完整格式:删掉上午做的「一排卡片 shelf」,改为卡片**列表** + 点新增弹**新建八字 modal**。
- **卡片列表**:`#ml-grid` 自适应网格(repeat(auto-fill,minmax(230px,1fr))),卡片=性别图标+名称+主命 chip+四柱(五行染色)+标签 chip+出生日期;顶部加**搜索框**(按名称/四柱/日期/标签过滤)。
- **新建八字 modal**:把整个 `<form id="f">` 移进 `#ml-modal`(role=dialog/aria-modal),`.studio` 只剩 aside。字段:阳历/阴历 **两 tab**(四柱 tab 未做——干支→日期多对一无法唯一定日,接不了真排盘)、名称、出生城市(+地图占位)、拨轮日期(复用原 DateWheel,未改)、出生时间、婚姻、时区下拉、性别、**标签 chips**(家人/朋友/伴侣/子女/宠物/同事/客户+自定义)、取消/确认。开:新增(达上限弹升级);关:✕/取消/backdrop/ESC/排盘成功。
- **接入真实功能**:确认→复用原提交链路(阳历/阴历经 DateWheel→#f-date 公历契约不变)→/api/ziwei→渲染+自动存卡(entry 含 name/tags/timezone)。

### 关键决策 / 注意
- 四柱 tab **未做**(接不了真排盘,不假装);地图**存档位**(城市文本输入为真实控件,不接外部瓦片以免 CSP/离线炸)。
- DateWheel 未改,只在开弹窗时 relabel 一次;实测弹窗内 1996/8/12 正确居中显示(subagent 担心的隐藏态初始化问题已验证 OK)。
- 由 general-purpose subagent 执行结构改造,主 loop 全程序化验证。

### 验证(Playwright)
- 空态/新增开 modal(display flex)/2 tab(无四柱)/名称·时区·8 标签 chip 齐/拨轮弹窗内正确显示 ✓
- 确认排盘→modal 关→卡片建(名称正确)/count 1/3 ✓;阴历 tab 提交成功 ✓;搜索过滤 ✓;点卡还原(命盘+名称回填)✓;删除 ✓
- 1280 + 375 双视口无横向溢出、modal 移动端可滚、ESC 关、console 零错 ✓;npm test 43/43 ✓;截图确认卡片/弹窗贴合 MetaSight 且融入墨韵。
- 改动未提交,等用户拍板。

### 下一步
1. 地运页(房屋库)同 modal 模式。
2. 四柱 tab / 地图选点 若要做需新后端能力。
