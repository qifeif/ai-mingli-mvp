/**
 * SplashCursor · 鼠标流体特效(WebGL 流体模拟)
 * ---------------------------------------------------------------
 * 源自 React Bits 的 <SplashCursor/>(React + JSX)。心易是纯手写 HTML+原生 JS、
 * 无框架无构建,故这里把 React 那层壳(useEffect 挂 canvas + 绑事件)剥掉,
 * 改写成一个自包含的原生 JS:调 initSplashCursor(opts) 即在页面顶层注入
 * 一张 fixed、pointer-events:none 的全屏 canvas,跑同一套 WebGL 流体着色器。
 *
 * 用法(页面 body 末尾):
 *   <script src="/splash-cursor.js"></script>
 *   <script>window.initSplashCursor && initSplashCursor();</script>
 * 传参覆盖默认见下方 defaults(照原组件 props)。
 *
 * 注意:z-index 50 —— 在页面内容之上、弹窗(z-index 80)之下;pointer-events:none 不挡点击。
 * prefers-reduced-motion 下自动不启动(尊重降载偏好)。
 */
(function () {
  'use strict';

  function initSplashCursor(userOpts) {
    // 降载:用户偏好减少动效则不启动(避免持续 rAF + WebGL 开销)
    if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
    if (document.getElementById('splash-cursor-canvas')) return; // 幂等:已注入则跳过
    ...
    const defaults = {
      ...
      TRANSPARENT: true,
      // 墨韵默认:关彩虹、走浅灰蓝单色(接替旧 ink.js 的鼠标墨迹)。
      RAINBOW_MODE: false,
      COLOR: '#B8C1C8',
    ...
    // ── 注入宿主容器 + canvas(等价原组件的 return JSX)──
    const host = document.createElement('div');
    host.id = 'splash-cursor-host';
    host.style.cssText = 'position:fixed;top:0;left:0;z-index:50;pointer-events:none;width:100%;height:100%;';
    ...
    if (!gl) { host.remove(); return; } // WebGL 不可用则安静退出
    ...
    function hexToRGB(hex) {
      ...
      const k = config.INK_INTENSITY != null ? config.INK_INTENSITY : 0.15;
    ...
    // 提供销毁句柄(SPA/热重载时可调,普通多页站点用不到,但保留干净退出能力)
    window.destroySplashCursor = function () {
      isActive = false;
